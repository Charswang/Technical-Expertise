...
- 论文修改说明【实际系统应用，其他三个用不用写？】【周一】【签字】
- 访问命中率图和分析的修改【周一】
- 最佳性的成本计算、实际系统应用【周二周三】
- 答辩PPT、思考问题【周四、周五、周六、周日】

> 老师，请问《论文修改说明》里只需要写“修改后答辩”中的意见吗？“同意答辩”中的意见还需要做出说明吗？
...
<font color="red">**实验中图的编号要改，还没改呢。**</font>

---

**<font color="red">(1)</font>**  第2章介绍了基于统计方法的热度识别和基于数据结构的热度识别的几个方法，但**<font color="red">没有将论文提出的数据的热度识别方法与这些方法进行比较；</font>**<font color="orange">**【还得改实验分析和图的数据】**</font>

> **论文修改说明：**
>
> 1、针对该意见，分析多种热度识别方法，增加了文章中所提出的热度识别机制分别与LRU、LFU以及Double LRU的对比。评估了这几种识别方法在不同数据集下的访问命中率的情况，并对实验结果进行分析。修改的内容主要在36页14行~37页16行之间所添加的针对访问命中率的实验分析部分，以及34页中12~15行、35页中20~23行的实验目标与方法部分。
...
> 因基于统计方法的数据热度识别方法往往不能很好的获取数据热度的变化，而基于数据结构的热度识别方法可以一定程度上识别数据热度的变化。因此，为了评估HEC中热度识别机制在识别数据热度上的实时性与准确性，本文通过计算数据访问命中率:HitRatio来对其进行评估。

> 首先为了验证HEC中的数据热度识别机制在热度识别上的实时性与准确性，本文将该热度识别机制与部分基于数据结构的热数据识别机制进行对比。本文选取经典的LRU、LFU与Double LRU算法来与本文方法进行对比，并从数据访问命中率的角度来进行评估。

【不同数据集下的，不同缓存容量下的结果？--  4个图】  /  【zipf数据集下，不同缓存容量下的结果？】【热温队列的分布怎样，】

<font color="red">**实验中图的编号要改，还没改呢。**</font>

<font color="red">**图还没画呢**</font>

【分析说明，要怎么说？】--  当是hec最高，然后再是doublelru与lfu交叉、lru

1、分析随着热度识别队列长度下不多增加

2、说明四种识别机制谁最强，谁最弱，平均减少/增加x%，原因

3、不同类型的数据集中，有些表现较好，有些表现较差，原因

10：00开始吧

【2023-5-7 上午：写完热度识别机制的实验描述、排版（**后面再考虑要不要加zipf0.8还和1.2的比较**）】

【2023-5-7 下午：思考最佳性说明、思考如何应用、写《论文修改说明》、下午来了先些论文修改说明，然后画图，然后考虑最佳性那里的成本】

【】

(2)  论文宣称提出的基于数据热度的存储方式分配机制可为不同热度场景下的数据分配最佳存储方式，但论文没有定义何为最佳存储方式和论证其最佳性，最好也能给出适当的例子；<font color="red">**没搞好，还得改，还没有计算具体的存储方式所消耗的成本**</font>**【要不说是一个数据在其生命周期的三个阶段的场景，三个阶段分别别是：频繁访问->访问次数减少->几乎不再访问，然后计算其成本】**

> 2、**论文修改说明：**
>
> 针对该意见，从访问特性的角度出发，将最符合数据访问特性的存储方式定义为最佳存储方式。并根据不同存储方式的优势举例说明不同访问特性的数据所适合的存储方式。为验证不同类型数据的最佳存储方式的最佳性，从数据消耗的存储成本和访问成本出发，分别选取频繁被访问、访问次数有明显减少以及几乎不再被访问的数据，分别计算在固定时间内使用不同存储方式时所消耗的成本。通过计算成本来验证最佳存储方式的最佳性。修改的内容主要分布在第10页24行~第11页21行之间。
...
>
> **论证举例：**直接成本角度举例说明，来表示.出选择出的存储方式的最佳性。【成本计算单价表格、数据访问情况表格？、一段话计算并分析说明】，在某个数据集中拿去四种访问情况的数据的例子？-- 热度极高[none]、热度中级[lz4]、热度低级[访问的很少gzip]、热度极低[没有被访问过bzip2]
...
(3)  论文工作是应用性较强的，但论文没有**讨论如何将提出的方法具体应用到实际系统中**。**<font color="red">【Ceph还是HDFS？】</font>**

【类似实现过程那种？】单副本、hdfs流式存储、往hdfs中引入其他不同的存储机制+引入方法--[接口啥的？]？、

【还是说加个应用？加个5.4 系统应用？】应用到hdfs中，说明一些应用的过程。然后展示效果：展示效果的话，那要怎么展示？直接说明与默认的hdfs配置进行对比？压缩率、访问延迟以及均衡性。

> 3、**论文修改说明：**
>
> 针对该意见，论文忽略了对相关软件环境与存储系统的详细描述，因此在第五章节的实验环境准备部分增加了对相关存储系统的描述以及存储系统相关的详细配置情况。另外，在描述实验方法的部分增加了如何将框架应用到相关存储系统的相关操作与流程。修改内容分布在第33页的7~12行、第35页的18~21行。
>
> 针对该意见，论文忽略了对相关软件环境与存储系统的详细描述，因此，在第五章节的实验环境部分增加了将HEC应用到HDFS存储系统的相关配置的描述，并在描述实验方法的部分增加了将HEC应用到HDFS存储系统的相关操作与流程。修改内容部分主要分布在第33页的7~12行、第35页的18~21行。
>
> 针对该意见，增加了将HEC应用到HDFS存储系统时的相关配置以及操作流程的阐述。并在不同的测试数据集下，针对不同评估指标进行了分析。

> **在实验部分进行说明在HDFS/OBS中完成原型的构建？/ 说明是在亚马逊云存储的hdfs或obs中进行应用的效果**
>
> **说明：**介绍HDFS/OBS的背景，然后说明在此基础之上完成实验和测试？
>
> **文章之前忽略了介绍所应用的系统，修改：文章中实验部分讨论了将在实现HEC框架的基础上，以HDFS/ceph为例，在其上使用HEC之后的效果**
>
> 【实在不行就直接说之前用的是hdfs，但是没有对其配置进行过多的描述，增加了部分对于实验环境所应用存储软件的说明，并分析其效果】【<font color="orange">**实在不行就这样吧，一切随缘吧，走一步看一步算了**</font>】【说一些对hdfs里面的配置？】
>
> 怎么讨论？字数要写多一点吗？
>
...
>
> 如果说要应用到系统中的话，那要怎么讨论呢？
>
> 【看看能不能做一下实验，然后通过实验告诉用户说哪个方法比较好，或者证明你的方法比较好，然后说已经怎么去应用在某些的系统里面，应用的效果怎么样。】
>
> 【将HEC机制应用到对象存储系统中，相比其默认存储机制而言，提升多少存储率，增加了多少访问延迟，提高了多少均衡性？】
>
...
> **说明**
>
> 热度主要是结合数据的历史访问频率与最近访问时间的局部性来进行评判。其中对历史访问频率而言，访问频率越高，未来被访问的概率越大。而对最近访问的时间局部性而言，当前被访问的数据在一段时间内可能会再次被访问。通过两者和衰减系数的计算来评估数据的当前热度。
...
> **说明**
>
> 在分布式多中心场景下，可能会导致热度识别不准确，使得部分数据在热度等级升高或降低时不能够及时切换其存储方式。

 3）在数据样本结构类似、访问频率分布均衡的场景下，本文方法实际效果如何？建议实验进一步补充分析。【这个其实也得改。。。。】【2023-5-8  简单改一下，确定更改的行数，好写在论文修改说明中】

针对该意见，在第五章节的实验分析中，进一步对ZIPF0.8以及ZIPF1.2这两种样本结构类似，但访问频率分布不同的数据集分别在访问命中率、压缩率、访问延迟以及均衡性上的表现进行对比分析。具体修改部分主要在xx
...
> **只改与固定压缩相比较吧**
>
> **压缩率部分：**而相比之下，在访问分布更为集中的zipf1.2的测试中，HEC的压缩率更高。这是因为部分热度低的数据在访问后期几乎不被访问，也导致这部分数据所使用的存储方式的压缩率更高。
>
> 

# 最晚到周三改完，剩余四天要搞ppt和思考问题。
